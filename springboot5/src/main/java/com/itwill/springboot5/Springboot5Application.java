 ...
 * 엔터티의 생성/수정 시간을 자동으로 기록하기 위해서 JPA Auditing기능을 활성화 시킴.
 * (1) Main class에서 @EnableJpaAuditing 설정
 * 
 * (2) 날짜/시간(LocalDate/LocalDateTime) 필드를 갖는 엔터티(BaseTimeEntity)에서
 * 		@EntityListeners(AuditingEntityListener.class) 설정
 * 
 * (3) 날짜/시간 필드(createdTime, modifiedTime)에 @CreatedDate, @LastModifiedDate 설정
 * */
