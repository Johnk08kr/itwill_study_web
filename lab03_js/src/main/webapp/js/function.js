...
// JS 모든 함수는 argument 속성(property)을 가지고 있음.
function testArgs() {
...
// 숫자 2개를 argument로 전달 받아서 뺄셈 결과를 리턴하는 함수
...
result = multiply(2, 3); // argument를 전달하면 기본값은 무시
console.log(`result = ${result}`);

result = multiply(2); // argument를 전달하지 않으면 기본값이 사용됨.
console.log(`result = ${result}`);

/** 
 * JS 함수는 객체(object)!!
 * 1. property를 가질 수 있음. (예) arguments
 * 2. 변수에 저장할 수 있음.
 * 3. 다른 함수를 호출할 때 argument로 함수(객체)를 전달할 수 있음.
 * 4. 함수 내부에서 다른 함수를 선언(정의) 할 수 있음.
 * 5. 함수 (객체)를 리턴할 수 있음. 
 */

const plus = add; // 함수 객체 add를 변수 plus에 할당(저장)
console.log(plus); // 함수 객체
console.log(plus(100, 200)); // 함수 호출
...
// 콜백(callback): (나중에 호출하기 위해서) 함수의 argument로 전달하는 함수.
